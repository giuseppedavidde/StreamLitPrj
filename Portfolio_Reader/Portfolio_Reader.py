...
if df_portfolio is not None:
    ...
    if isinstance(stock_under_test, pd.DataFrame):
        ...
        percentage_return = (total_gain/invested_capital_invested)*100 if invested_capital_invested else 0
        ...
        with st.expander("Show Weights by Symbol in Each Category Pie Charts"):
             for category in capital_by_category.keys():
                ...
                # Logic to filter
                cat_syms = [s for s, d in weight_by_symbol.items() if d["category"] == category]
                cat_ws = [d["weight"] for s, d in weight_by_symbol.items() if d["category"] == category]
                
                cat_ws_today = [d["weight"] for s, d in weight_by_symbol_today.items() if d["category"] == category]
                
                if cat_syms and sum(cat_ws) > 0:
                     st.markdown(f"**{category}**")
                     col_a, col_b = st.columns(2)
                     with col_a:
                         f3 = safe_create_pie_chart(cat_syms, cat_ws, f"BEGINNING: {category}")
                         if f3: st.plotly_chart(f3, use_container_width=True)
                     with col_b:
                         f4 = safe_create_pie_chart(cat_syms, cat_ws_today, f"TODAY: {category}")
            ...
            if temp_ticker is not None:
                ...
                if not hist.empty:
                    ...
                    if last_trading_day < end_date_dca:
                        st.warning(f"End date adjusted to last trading day: {last_trading_day}")
        ...
        if category is None:
             category = st.sidebar.selectbox("Category", ["ETF", "Crypto", "Stock"], key="single_stock_category_fallback")
        ...
        if (
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=stock_data_dca_values.index, y=stock_data_dca_values["stock_price"], mode="lines", name="Price"))
            fig.add_trace(go.Scatter(x=stock_data_dca_values.index, y=stock_data_dca_values["MA200"], mode="lines", name=f"MA{ma_period}"))
            fig.update_layout(title=f"{stock_under_test} Analysis", hovermode="x unified")
