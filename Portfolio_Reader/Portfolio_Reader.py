...
# --- LLM Chatbot Section ---
st.sidebar.header("Conversazione con LLM (Gemini)")
GEMINI_API_KEY = get_gemini_api_key()
init_chat_state()
if st.sidebar.button("Nuova Chat", key="new_chat_gemini"):
    new_chat()
user_prompt = st.sidebar.text_area("Tu (ChatBot):")
if GEMINI_API_KEY and st.sidebar.button("Invia", key="send_gemini") and user_prompt:
    ...
    st.session_state.chat_time_end = dtm.now()
render_chat()

# --- File Upload Section ---
st.sidebar.header("Portfolio Data Upload")
portfolio_source = st.sidebar.radio(
    "Portfolio Source:", ["Upload CSV", "GitHub URL"], index=0
)
df_portfolio = None
...
if df_portfolio is None:
    ...
    default_path = os.path.join(
        os.path.dirname(__file__), "..", "Data_for_Analysis", "My_Portfolio.csv"
    )
    if os.path.exists(default_path):
        ...
        st.warning(
            "Please upload a portfolio CSV file or provide a GitHub URL to continue."
        )
        st.stop()
...
if df_portfolio is not None:
    with st.expander("Edit Portfolio Data", expanded=False):
        ...
        st.markdown(
            "#### Edit your portfolio below. You can enter formulas (e.g., =A+B or =SUM(A:C)) in the Formula column. After editing, you can save changes directly to your GitHub repository."
        )
        ...
        edited_df["Formula Result"] = edited_df.apply(
            lambda row: eval_formula(row, edited_df), axis=1
        )
        st.dataframe(edited_df)

        csv_data = edited_df.drop(columns=["Formula Result"]).to_csv(
            index=False, sep=";"
        )
        ...
        with st.expander("GitHub Settings (required for saving)"):
            ...
            github_repo = st.text_input(
                "GitHub Repository (owner/repo)", value="your-username/your-repo"
            )
            github_file_path = st.text_input(
                "Path to CSV in repo (e.g. Data_for_Analysis/My_Portfolio.csv)",
                value="Data_for_Analysis/My_Portfolio.csv",
            )
        ...
        st.info(
            "You can use formulas like =A+B or =SUM(A:C) in the Formula column. Your GitHub token is only used to update the file and is not stored anywhere."
        )
...
if portfolio_mode == "Portfolio":
    ...
    start_date = st.sidebar.date_input(
        "Start Date", value=today.replace(year=today.year - 3)
    )
else:
...
if isinstance(stock_under_test, pd.DataFrame):
        ...
        if symbol != "USDEUR=X" and pd.notna(symbol):
            ...
            if shares_info is not None:
                if invested_capital_usd_dict[symbol] > 0.0:
                    shares_value_dict[symbol] = (
                        shares_info["Close"].iloc[-1] * latest_rate
                    )
    ...
    with st.expander("Show Invested Capital by Category Pie Charts"):
        ...
        fig1 = safe_create_pie_chart(
            ...
            "BEGINNING: Capitale Totale Investito per Categoria",
        )
        if fig1 is not None:
        ...
        fig2 = safe_create_pie_chart(
            ...
            "TODAY: Capitale Totale Investito per Categoria",
        )
        if fig2 is not None:
    ...
    with st.expander("Show Weights by Symbol in Each Category Pie Charts"):
        for category in capital_by_category.keys():
            ...
            if category_symbols and category_weights and sum(category_weights) > 0:
                st.markdown(
                    f'#### BEGINNING: Peso Percentuale per Simbolo in Categoria "{category}"'
                )
                fig3 = safe_create_pie_chart(
                    category_symbols,
                    category_weights,
                    f'BEGINNING: Peso Percentuale per Simbolo in Categoria "{category}"',
                )
            ...
            if (
                category_symbols
                and category_weights_today
                and sum(category_weights_today) > 0
            ):
                st.markdown(
                    f'#### TODAY: Peso Percentuale per Simbolo in Categoria "{category}"'
                )
                fig4 = safe_create_pie_chart(
                    category_symbols,
                    category_weights_today,
                    f'TODAY: Peso Percentuale per Simbolo in Categoria "{category}"',
                )
        ...
        if temp_ticker is not None:
            ...
            if not hist.empty:
                ...
                if last_trading_day < end_date_dca:
                    st.warning(
                        f"End date adjusted from {end_date_dca.strftime('%Y-%m-%d')} to last available trading day: {last_trading_day}"
                    )
                    end_date_dca = last_trading_day
            else:
                st.warning(
                    "Could not determine last trading day, using selected end date."
                )
        else:
            st.warning(
                "Could not retrieve ticker data. Please check the ticker symbol."
            )
    ...
    if category is None:
        category = st.sidebar.selectbox(
            "Category", ["ETF", "Crypto", "Stock"], key="single_stock_category"
        )
    # Get stock data
    ...
    if (
        ...
        st.error(
            "No valid stock price or MA200 data found for this ticker and date range. Please check the ticker symbol and date range.\n"
        ...
        fig.add_trace(
            go.Scatter(
                x=dates, y=stock_data_dca_values["MA200"], mode="lines", name="MA200"
            )
        )
        fig.update_layout(
...
# --- Footer ---
st.caption(
    "This dashboard is for informational purposes only. Always consult a financial advisor for investment decisions."
)
