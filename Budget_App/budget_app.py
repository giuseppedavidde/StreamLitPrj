...
def calculate_metrics(df):
    ...
    df["Risparmio %"] = df.apply(
        lambda row: (
            (row["Reddito meno spese"] / row["Totale Entrate"] * 100)
            if row["Totale Entrate"] != 0
            else 0
        ),
        axis=1,
    )
...
if not df.empty:
    ...
    if page == "Dashboard":
        ...
        fig_trend.add_vline(
            x=selected_date_obj.timestamp()
            * 1000,  # Plotly a volte preferisce timestamp ms per assi date
            line_width=1,
            line_dash="dash",
            line_color="white",
        ...
        fig_trend.update_layout(
            ...
            xaxis_title="",
            yaxis_title="€ Euro",
            xaxis=dict(type="date"),  # Assicura che l'asse sia trattato come data
            hovermode="x unified",
        ...
        with st.container(border=True):
            ...
            df_trend_all = df.sort_values(
                "DateObj"
            )  # Tutto lo storico per il trend di lungo periodo
